...
date: "2026-02-16"
categories: [shell, macos, git, neovim]
...
| # | Layer              | Component(s)                    | Responsibility                                              | Configurable via   |
|--:|--------------------|---------------------------------|-------------------------------------------------------------|--------------------|
| 1 | Terminal emulator  | Kitty, Ghostty, WezTerm         | Window drawing, input capture, font shaping, GPU rendering  | `kitty.conf`       |
| 2 | Window manager     | Quartz Compositor (WindowServer)| Window compositing, Spaces, multi-display routing           | macOS Settings     |
| 3 | PTY                | `/dev/ttys*` kernel device      | Bidirectional pipe pairing the emulator to the shell        | not user-tunable   |
| 4 | Shell              | Zsh                             | Command parsing, history, completion, scripting, job control| `.zshrc`, `.zshenv`|
| 5 | System calls       | POSIX (`open`, `read`, `fork`, `exec`, `ioctl`) | Kernel API surface invoked by every program | not user-tunable   |
| 6 | OS frameworks      | AppKit, Metal, Core Text, IOKit | Windowing, GPU pipelines, font resolution, device access    | not user-tunable   |
| 7 | Userland           | Darwin (BSD libraries)          | POSIX-compliant C library and command set                   | not user-tunable   |
| 8 | Kernel             | XNU (Mach + BSD)                | Process scheduling, memory, filesystem, networking, drivers | not user-tunable   |
| 9 | Hardware           | Apple Silicon                   | CPU, GPU, display panel, keyboard, storage                  | not user-tunable   |

: The macOS terminal stack, ordered from user-facing
(layer 1) to hardware (layer 9). A single keystroke originates
at the emulator, descends through the PTY (layer 3) into the
shell (layer 4), which translates the parsed command into POSIX
system calls (layer 5) that the kernel (layer 8) executes
against hardware (layer 9). Output flows back along the same
path. Only layers 1, 2, and 4 are routinely user-configurable;
this post addresses layers 1 (`kitty.conf`) and 4 (`.zshrc`,
`.zshenv`). The intermediate layers are listed for completeness
and because debugging shell behaviour occasionally requires
reasoning about them, for example a hung PTY that survives the
emulator quitting, or a Nerd Font that fontconfig sees but Core
Text cannot resolve. {#tbl-terminal-stack}
...
More formally, this post documents the Shell layer (Layer 4) of
the Workflow Construct described in
[post 52](/posts/52-workflow-construct/). zsh is the choice of
shell at this layer; the layer's tool is replaceable (with
bash, fish, or nushell), but the layer itself, the contract
between the user's keystrokes and every script and command
they run, is not. The configuration files documented here are
the operative artefacts that instantiate the Shell layer on a
given machine.
...
A caveat: sourcing `~/.env` exports every credential into the
environment of every child process, which is the simplest
arrangement but the loosest. A safer pattern is per-process
injection via a `with-secret` wrapper backed by `pass` (the
Unix password store), which decrypts a credential only at the
moment a specific command needs it and leaves the surrounding
environment clean. The `~/.env` approach above is fine for solo
research projects on a trusted machine; for anything more
exposed, see [post 55](/posts/55-secrets-management/) for the
full three-tier scheme (`pass` for API keys, AWS IAM Identity
Center for AWS credentials, `gitleaks` at staging as a residual
safety net).
...
```sh
: ${PRJ_ROOT:=$HOME/Dropbox/prj}
cdpath=($PRJ_ROOT $PRJ_ROOT/.. $PRJ_ROOT/../sbx $PRJ_ROOT/../work)
```
...
`$PRJ_ROOT/myproject` automatically. Combined with `setopt
auto_cd` (which permits typing a directory name without `cd`),
navigating between analysis projects becomes a single word
rather than a full path.

The `$PRJ_ROOT` indirection is deliberate. Hardcoding
`~/Dropbox/prj` in `cdpath` (and in every backup script and
shell function that follows) makes the workflow load-bearing
on Dropbox, which is increasingly the wrong default. Setting
`PRJ_ROOT` once in `~/.zshenv` lets the same configuration
follow the project tree to a non-cloud location with a
single-line edit. See [post 64](/posts/64-migrating-off-dropbox/)
for the full migration argument and a worksheet for moving off
Dropbox without breaking a running pipeline.

The trade-off with `cdpath` is name collisions: if two
directories contain subdirectories with the same name, Zsh
picks the first match. Keeping project names unique across the
listed directories avoids surprises.

### Modern CLI Replacements

Several Rust-implemented utilities have superseded the GNU/BSD
core tools that ship with macOS and Linux. They render faster,
respect `.gitignore` by default, produce coloured output
suitable for interactive use, and integrate with `fzf` more
cleanly than their predecessors. The block below wires the
common set into Zsh:

```sh
# zoxide: cd with a frequency table; '--cmd j' aliases the
# jump command to 'j' so existing autojump muscle memory carries
# over.
...
# bat: cat with syntax highlighting; alias only for interactive
# use so that scripts calling 'cat' continue to see plain output.
...
# lazygit: full-screen TUI for git; not aliased to 'git' so the
# plain command stays available.
if command -v lazygit > /dev/null 2>&1; then
  alias lg='lazygit'
fi
```

Two notes on the migration. First, `zoxide` supersedes
`autojump`: it maintains its own frequency database and is
faster on large histories. Bind it to `j` (as above) and the
existing muscle memory carries over; the autojump database can
optionally be seeded into zoxide with a one-time `awk + xargs`
script. Second, `delta` requires a corresponding `[core] pager
= delta` entry in `~/.gitconfig` (covered in the Git section
below). The full reference, including the autojump-to-zoxide
migration script and per-tool failure modes, is in
...
`gz` is the minimal version of this idea. A more complete
implementation lives in
[post 49](/posts/49-zzgit/) as `zzgit`, which adds
`gitleaks`-based secret scanning at staging time, a richer
fzf-driven hunk picker, and an undo path for the inevitable
mis-staged file. Readers who plan to commit anything that ever
touches an API key should treat `gz` as a starting point and
graduate to `zzgit`.
...
plugin loading (`zsh-autosuggestions`,
`zsh-syntax-highlighting`) with conditional guards, ls/grep
colour aliases, application shortcuts, directory stack
navigation, NVM loading, and conda initialisation. These are
standard shell configuration; the
...
![Version control configuration.](media/images/ambiance1.png){.img-fluid}
...
![Line editing and modal input.](media/images/ambiance1.png){.img-fluid}
...
1. Open Kitty and type `prj` (thanks to `cdpath`, this expands
   to `$PRJ_ROOT`)
2. Type `j analysis` and zoxide jumps to the most-frequented
   directory matching `analysis`
...
The individual pieces (cdpath, zoxide, fzf functions, the git
...
|-----------------|--------|
| `prj` | Jump to `$PRJ_ROOT` via cdpath |
| `j <partial>` | Zoxide jump to most-frequented matching directory |
| `ll` / `la` / `lt` | eza listings (long, all, tree) |
| `lg` | Open lazygit TUI in current repo |
...
zsh --version
echo $SHELL
for t in zoxide eza bat fd rg fzf delta lazygit; do
...
2. **Plugin startup cost.** Loading `zsh-syntax-highlighting`,
   `zsh-autosuggestions`, and `zoxide init` adds roughly
   100-200ms to shell startup. If lag appears when opening new
...
6. **`fd` hides hidden files by default.** Unlike `find`, which
   walks dotfiles unless told otherwise, `fd` skips hidden files
   and `.gitignore`d paths by default. Add `-H` to include
   hidden files and `-I` to ignore `.gitignore`. The defaults
   are usually what you want interactively, but a `fd
   .credentials` that returns nothing is a common surprise.

7. **Debian package names for the modern CLI tools.** On Ubuntu
   and Debian, `fd` is packaged as `fd-find` and `bat` as
   `batcat` to avoid conflicts with older binaries. Either
   create personal symlinks (`ln -sf "$(command -v fdfind)"
   ~/.local/bin/fd`) or alias them in `.zshrc`. Without this
   step, the aliases above silently fail.
...
brew uninstall zoxide fzf eza bat fd ripgrep git-delta lazygit  # macOS
```
...
- Directory navigation is a solved problem in Zsh. Between
  `cdpath`, `auto_pushd`, and `zoxide`, there is no reason to
  type full paths.
...
- Aliasing `cat` to `bat` is convenient interactively but
  changes output for any script that calls `cat` from the same
  shell. Pipelines that parse cat output (e.g.,
  `cat file | head`) can break if `bat` decides to add headers.
  Keep `bat --style=plain --paging=never` in the alias to
...
- The `.zshrc` includes hardcoded paths (e.g.,
  `/opt/miniconda3`, plugin install directories) that will not
  work on other machines without editing. The `$PRJ_ROOT`
  pattern shown above for project paths should be extended to
  these as well.
...
five things first is recommended: a Nerd Font for icon support,
`cdpath` plus `zoxide` for directory navigation, `fzf` for
fuzzy file finding, the modern CLI replacements (`eza`, `bat`,
`fd`, `ripgrep`, `delta`) for everyday display and search, and
`.inputrc` for vi-mode consistency. These changes make the
...
- [The Workflow Construct](/posts/52-workflow-construct/): the
  layered architecture this post instantiates at the Shell layer
- [Modern CLI Replacements](/posts/53-modern-cli-replacements/):
  the full reference for `eza`, `bat`, `fd`, `zoxide`, `delta`,
  `lazygit`, with installation, aliases, and per-tool failure
  modes
- [Secrets Management for Data Scientists](/posts/55-secrets-management/):
  the three-tier scheme (`pass`, AWS IAM Identity Center,
  `gitleaks`) that supersedes the `~/.env` approach used here
- [zzgit: Interactive Git Staging with Secret
  Scanning](/posts/49-zzgit/): the production version of the
  `gz` function, with `gitleaks` integration and an fzf-driven
  hunk picker
- [LLM-Augmented Editing](/posts/60-llm-augmented-editing/):
  how the shell layer described here integrates with Claude
  Code and similar tools
- [Migrating Off Dropbox](/posts/64-migrating-off-dropbox/):
  the rationale for the `$PRJ_ROOT` indirection used in the
  `cdpath` block
...
- Optional: `fzf`, `ripgrep`, `fd`, `bat`, `eza`, `zoxide`,
  `delta`, `lazygit`, `trash-cli`
...
---
*Rendered on 2026-05-10 at 17:10 PDT.*<br>
*Source: ~/prj/qblog/posts/01-configtermzsh/configtermzsh/analysis/report/index.qmd*