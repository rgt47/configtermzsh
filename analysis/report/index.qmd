...
*The rhythm of configuration: small decisions that compound into
a seamless workflow.*
...
*Configuration as architecture: each layer built deliberately on
the one below.*
...
I would rather have a working shell with a missing plugin than a
broken shell.
...
*The destination: a command line that works with you, not
against you.*
...
`.inputrc`. When something breaks, I know exactly where to look.
When I want to add something new, I know exactly where to put it.

If you are starting from a default terminal setup, I would
suggest focusing on four things first: a Nerd Font for icon
support, `cdpath` for directory navigation, `fzf` for fuzzy
file finding, and `.inputrc` for vi-mode consistency. Those
changes made the biggest difference in my daily workflow.
...
- [GitHub CLI](https://cli.github.com/) -- the `gh` tool used
  for authentication in this configuration
